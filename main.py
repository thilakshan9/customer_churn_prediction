...
def prepare_input(credit_score, location, gender, age, tenure, balance,
  ...
  input_dict = {
      ...
      'Geography_Female': 1 if gender == "Female" else 0
  }
...
def make_predictions(input_df, input_dict):
  ...
  with col1:
    ...
    st.write(f"The customer has a {avg_probability:.2f}% chance of churning.")
...
def explain_prediction(probability, input_dict, surname):
  prompt = f"""
  You are an expert data scientist at a bank, where you specialise in interpretin and explaining predictions of machine learning models.

  Your machine learning model as predicted that a customer named {surname} has a {round(probability * 100, 1)}% probability of churning, based on the information provided below.
...
def generate_email(probability, input_dict, explanation, surname):
  prompt = f"""
  You are a manager at a prestigious bank. You are responsible for ensuring customers stay with the bank and are incentivised with various offers.
